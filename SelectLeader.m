...
function rep_h=SelectLeader(rep,beta)
    ...
    %occ_cell_member_count,occupied grid's particle number
    ...
    for i=1:2
    selected_cell_members=find(GridIndices==selected_cell_index(i));
    
    n=numel(selected_cell_members);
    
    selected_memebr_index=randi([1 n]);
    
    h(i)=selected_cell_members(selected_memebr_index);
    end
    rep_h=rep(h);
end